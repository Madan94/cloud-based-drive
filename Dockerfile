...
# Download mime.types - try GitHub mirror first (more reliable), then SVN as fallback
# BusyBox wget doesn't support --tries or --retry-connrefused, so we use -T for timeout
RUN for url in \
    "https://raw.githubusercontent.com/apache/httpd/trunk/docs/conf/mime.types" \
    "https://svn.apache.org/repos/asf/httpd/httpd/trunk/docs/conf/mime.types"; do \
    wget -T 30 -O /etc/mime.types "$url" && break; \
    done
